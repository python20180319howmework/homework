...
def main():
    ...
    bomb1 = enemy.Bomb(bg_img.get_rect())
    myplanenum = 3
    running = True
    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.button == 1 and (width-again_img.get_rect().width)/2 < pygame.mouse.get_pos()[0] < (width-again_img.get_rect().width)/2 + \
                        again_img.get_rect().width and height/2-again_img.get_rect().height-25< pygame.mouse.get_pos()[1] < height/2-again_img.get_rect().height-25 + \
                        again_img.get_rect().height:
                    pause_flag = False
                    myplane.reset()

                if event.button == 1 and (width-over_img.get_rect().width)/2 < pygame.mouse.get_pos()[0] < (width-over_img.get_rect().width)/2 +\
                        over_img.get_rect().width and height/2+over_img.get_rect().height+25< pygame.mouse.get_pos()[1] < height/2+over_img.get_rect().height+25 + \
                        over_img.get_rect().height:
                    running = False
            elif event.type == MOUSEMOTION:
                if pause_rect.left < pygame.mouse.get_pos()[0] < pause_rect.left + pause_rect.width \
                        and pause_rect.top < pygame.mouse.get_pos()[1] < pause_rect.top + pause_rect.height:
                    if pause_flag:
                        # 暂停
                ...
                if event.key == K_SPACE and bom_num > 0:
                    # 放炸弹
        ...
        if myplane.alive:
            ...
            if not delay % 10 and me_index <3:
                ...
                screen.blit(again_img,((width-again_img.get_rect().width)/2,height/2-again_img.get_rect().height-25))
                screen.blit(over_img,((width-over_img.get_rect().width)/2,height/2+again_img.get_rect().height+25))
                pause_flag = True
